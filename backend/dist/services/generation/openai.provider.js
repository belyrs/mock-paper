...
function modelControls(model, reasoningEffort, temperature) {
...
class OpenAiQuestionGenerationProvider {
    async generate(request) {
        if (request.questionCount <= env_1.env.OPENAI_GENERATION_BATCH_SIZE ||
        ...
        const planBatches = Array.from({
            length: Math.ceil(request.generationPlan.length / env_1.env.OPENAI_GENERATION_BATCH_SIZE),
        }, (_, index) => request.generationPlan.slice(index * env_1.env.OPENAI_GENERATION_BATCH_SIZE, (index + 1) * env_1.env.OPENAI_GENERATION_BATCH_SIZE));
        const batchRequests = planBatches.map((generationPlan) => {
            const difficultyCounts = generationPlan.reduce((counts, slot) => {
        ...
        logger_1.logger.info({
            ...
            batchSize: env_1.env.OPENAI_GENERATION_BATCH_SIZE,
            concurrency: env_1.env.OPENAI_GENERATION_CONCURRENCY,
        }, "Dispatching OpenAI question generation in focused batches.");
        const batchResults = [];
        for (let batchOffset = 0; batchOffset < batchRequests.length; batchOffset += env_1.env.OPENAI_GENERATION_CONCURRENCY) {
            const wave = batchRequests.slice(batchOffset, batchOffset + env_1.env.OPENAI_GENERATION_CONCURRENCY);
            const waveResults = await Promise.all(wave.map((batchRequest, index) => this.generateQuestionBatch(batchRequest, batchOffset + index + 1, batchRequests.length)));
            batchResults.push(...waveResults);
        }
        const sumMetric = (selector) => batchResults.reduce((total, result) => total + (result.metrics ? selector(result.metrics) : 0), 0);
        const questions = batchResults
            .flatMap((result) => result.questions)
            .map((question, index) => ({ ...question, questionNumber: index + 1 }));
        const historicalAnalysisModes = new Set(batchResults.map((result) => result.historicalAnalysisMode));
        logger_1.logger.info({
            provider: "openai",
            model: env_1.env.OPENAI_MODEL,
            subject: request.subject,
        ...
        return {
            rawPrompt: batchResults
                .map((result, index) => `Generation batch ${index + 1}:\n${result.rawPrompt}`)
                .join("\n\n"),
            rawResponse: {
            ...
            model: env_1.env.OPENAI_MODEL,
            questions,
            historicalAnalysisMode: historicalAnalysisModes.size === 1
                ? batchResults[0].historicalAnalysisMode
                : "mixed",
            historicalAnalysisSummary: Array.from(new Set(batchResults.map((result) => result.historicalAnalysisSummary))).join(" "),
            metrics: {
                provider: "openai",
                model: env_1.env.OPENAI_MODEL,
                ...
                usage: {
                    promptTokens: batchResults.reduce((total, result) => total + (result.metrics?.usage?.promptTokens ?? 0), 0),
                    completionTokens: batchResults.reduce((total, result) => total + (result.metrics?.usage?.completionTokens ?? 0), 0),
                    totalTokens: batchResults.reduce((total, result) => total + (result.metrics?.usage?.totalTokens ?? 0), 0),
                    cachedPromptTokens: batchResults.reduce((total, result) => total + (result.metrics?.usage?.cachedPromptTokens ?? 0), 0),
                },
            },
        };
    }
    async generateQuestionBatch(request, batchNumber, batchCount) {
        ...
        const response = await fetch(`${env_1.env.OPENAI_BASE_URL}/chat/completions`, {
            ...
            body: JSON.stringify({
                model: env_1.env.OPENAI_MODEL,
                ...modelControls(env_1.env.OPENAI_MODEL, env_1.env.OPENAI_GENERATION_REASONING_EFFORT, env_1.env.QUESTION_GENERATION_TEMPERATURE),
    ...
    async reviewQuestions(request, questions) {
        if (!env_1.env.OPENAI_API_KEY) {
            throw new app_error_1.AppError(503, "OPENAI_NOT_CONFIGURED", "OPENAI_API_KEY is required to review generated questions.");
        }
        ...
        const batches = Array.from({
            length: Math.ceil(indexedQuestions.length / env_1.env.OPENAI_REVIEW_BATCH_SIZE),
        }, (_, index) => indexedQuestions.slice(index * env_1.env.OPENAI_REVIEW_BATCH_SIZE, (index + 1) * env_1.env.OPENAI_REVIEW_BATCH_SIZE));
        const startedAt = Date.now();
        logger_1.logger.info({
            provider: "openai",
            model: env_1.env.OPENAI_REVIEW_MODEL,
            subject: request.subject,
            candidateCount: questions.length,
            batchCount: batches.length,
            batchSize: env_1.env.OPENAI_REVIEW_BATCH_SIZE,
            concurrency: env_1.env.OPENAI_REVIEW_CONCURRENCY,
        }, "Dispatching independent academic quality review in focused batches.");
        const batchResults = [];
        for (let batchOffset = 0; batchOffset < batches.length; batchOffset += env_1.env.OPENAI_REVIEW_CONCURRENCY) {
            const wave = batches.slice(batchOffset, batchOffset + env_1.env.OPENAI_REVIEW_CONCURRENCY);
            const waveResults = await Promise.all(wave.map((batch, index) => this.reviewQuestionBatch(request, batch, batchOffset + index + 1, batches.length)));
            batchResults.push(...waveResults);
        }
        const reviews = batchResults.flatMap((result) => result.reviews);
        const validIndexes = new Set(reviews.map((review) => review.candidateIndex));
        if (reviews.length !== questions.length ||
            validIndexes.size !== questions.length ||
            reviews.some((review) => review.candidateIndex < 1 || review.candidateIndex > questions.length)) {
            throw new app_error_1.AppError(502, "OPENAI_REVIEW_INVALID_OUTPUT", "The academic review did not include exactly one decision for every candidate.", { expected: questions.length, received: reviews.length });
        }
        const sumMetric = (selector) => batchResults.reduce((total, result) => {
    ...
    async reviewQuestionBatch(request, indexedQuestions, batchNumber, batchCount) {
        const reviewPayload = indexedQuestions.map(({ candidateIndex, question }) => ({
            candidateIndex,
        ...
        const candidatesByIndex = new Map(indexedQuestions.map(({ candidateIndex, question }) => [
            candidateIndex,
            question,
        ]));
        const prompt = [
            ...
            `Difficulty guidance: ${Object.entries(request.syllabusContext.difficultyGuidance ?? {})
                .map(([difficulty, guidance]) => `${difficulty}: ${guidance.join(" ")}`)
                .join(" | ") ||
        ...
        const response = await fetch(`${env_1.env.OPENAI_BASE_URL}/chat/completions`, {
            ...
            body: JSON.stringify({
                model: env_1.env.OPENAI_REVIEW_MODEL,
                ...modelControls(env_1.env.OPENAI_REVIEW_MODEL, env_1.env.OPENAI_REVIEW_REASONING_EFFORT, 0),
                response_format: REVIEW_RESPONSE_FORMAT,
                messages: [
                    {
                        role: "system",
                        content: "You are a strict independent academic examiner and answer-key auditor. Review every candidate, solve it independently, and return JSON only.",
                    },
        ...
        const content = parsed.choices?.[0]?.message?.content;
        if (!content) {
            throw new app_error_1.AppError(502, "OPENAI_REVIEW_EMPTY_RESPONSE", "The OpenAI academic review did not contain any content.");
        }
        let rawReview;
        try {
            ...
            throw new app_error_1.AppError(502, "OPENAI_REVIEW_INVALID_JSON", "The OpenAI academic review was not valid JSON.");
        }
        const reviews = (rawReview.reviews ?? []).map((review) => {
            ...
            const independentCorrectOption = ["A", "B", "C", "D"].includes(String(review.independentCorrectOption).toUpperCase())
            ...
            const verificationSummary = typeof review.verificationSummary === "string"
            ...
            const answerMatches = independentCorrectOption === candidate?.correctOption;
            ...
            if (!answerMatches) {
                issues.push(independentCorrectOption
        ...
        const expectedIndexes = new Set(indexedQuestions.map(({ candidateIndex }) => candidateIndex));
        const validIndexes = new Set(reviews.map((review) => review.candidateIndex));
        if (reviews.length !== indexedQuestions.length ||
            validIndexes.size !== indexedQuestions.length ||
            reviews.some((review) => !expectedIndexes.has(review.candidateIndex))) {
            throw new app_error_1.AppError(502, "OPENAI_REVIEW_INVALID_OUTPUT", "The academic review did not include exactly one decision for every candidate.", {
