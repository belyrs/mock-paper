...
function buildQuestionPrompt(input) {
    const counts = input.difficultyCounts ?? (0, difficulty_1.mixToCounts)(input.difficultyMix, input.questionCount);
    const difficultyDistribution = [
        `Easy: ${counts.Easy} question(s)`,
        `Medium: ${counts.Medium} question(s)`,
        `Hard: ${counts.Hard} question(s)`,
    ].join("; ");
    const compactList = (values, limit) => values
        .filter(Boolean)
        .slice(0, limit)
        .join("; ") || "none";
    const prompt = prompt_template_1.BASE_PROMPT
    ...
    const compactPlan = ["Easy", "Medium", "Hard"]
        .map((difficulty) => {
        const entries = Array.from(new Set(input.generationPlan
            .filter((slot) => slot.difficulty === difficulty)
            .map((slot) => `${slot.concept} (${slot.pattern})`))).slice(0, 3);
        if (entries.length === 0) {
            return null;
        }
