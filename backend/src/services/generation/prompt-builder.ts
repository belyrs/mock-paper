...
export function buildQuestionPrompt(input: GenerationProviderRequest) {
  const counts =
    ...
    mixToCounts(input.difficultyMix, input.questionCount);
  ...
  const compactList = (values: string[], limit: number) =>
    values.filter(Boolean).slice(0, limit).join("; ") || "none";

  const prompt = BASE_PROMPT.replace("[SUBJECT]", input.subject)
    ...
    .replace(
      "[TARGET EXAMS]",
      input.targetExams.map(targetExamLabel).join(", "),
    )
  ...
  const compactPlan = input.generationPlan
    .map(
      (slot) =>
        `${slot.slotNumber}. [${slot.difficulty}] Concept: ${slot.concept} | Task: ${slot.pattern} | Form: ${slot.questionForm} | Outcome: ${slot.learningOutcome}`,
    )
    .join("\n");
...
- Difficulty guidance: ${(["Easy", "Medium", "Hard"] as const).map((difficulty) => `${difficulty}: ${compactList(input.syllabusContext.difficultyGuidance?.[difficulty] ?? [], 5)}`).join(" | ")}
`.trim();
