...
const INTERNAL_TECHNOLOGY_REPLACEMENT_PATTERN = new RegExp(
  INTERNAL_TECHNOLOGY_PATTERN.source,
  "gi",
);

export function containsInternalTechnologyReference(value: string) {
  return INTERNAL_TECHNOLOGY_PATTERN.test(value);
}

export function redactInternalTechnologyReferences(value: string) {
  return value
    .replace(INTERNAL_TECHNOLOGY_REPLACEMENT_PATTERN, "question-paper platform")
    .replace(
      /\bquestion-paper platform\s+(?:model|provider|service|system|response|request|prompt)\b/gi,
