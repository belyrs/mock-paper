...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your MockPaper profile and generated paper history.",
      },
      { property: "og:title", content: "Profile — MockPaper" },
      {
        property: "og:description",
        content: "View your account details and past question papers.",
      },
    ],
  }),
...
function ProfilePage() {
  ...
  return (
    <div className="app-canvas min-h-[calc(100vh-4rem)]">
            ...
            {history.map((p) => (
              <li key={p.id} className="relative">
                ...
                <div className="absolute top-1/2 right-11 -translate-y-1/2 text-sm">
                  <PaperNameEditor
                    name={p.title}
                    onSave={(name) => renamePaper(p.id, name)}
                    nameHidden
                  />
                </div>
              </li>
